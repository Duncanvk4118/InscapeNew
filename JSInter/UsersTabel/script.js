...
function fGetHtml(users) {
  var htmlTable = "<table class='tblA txt_sm'>";
  htmlTable += "<tr>"; // plaats de row met headers van de tabel
  htmlTable += "<th>Id</th>";
  htmlTable += "<th>Naam</th>";
  htmlTable += "<th>Alcohol</th>";
  htmlTable += "</tr>";
  for (var i = 0; i < users.length; i++) {
    // loop door alle users plaats de row met headers van de tabel
    htmlTable += "<tr>";
    htmlTable += "<td>" + users[i].id + "</td>"; // haal van de array nr i van de users, het object key "name" op
    htmlTable += "<td>" + users[i].naam + "</td>"; // haal van de array nr i van de users, het object key "dob" (date of birth) op
    htmlTable += "<td>" + users[i].perc + "</td>"; // haal van de array het email adres op
    htmlTable += "</tr>";
  }
  htmlTable += "</table>";
  return htmlTable;
...
function getHtmlFromUrl(url) {
  // haal dat van URL binnen, verwerkt dit in een HTML-table en plaatst {
  fetch(url)
    .then((data) => {
      return data.json();
    })
    .then((json) => {
      ...
      console.log("beers = ", json.data); // de json bevat users en offices, laat hier alleen de users zien
...
function fGetData(url) {
  // haalt de data op van de url (API) en geeft met "return json" de data terug
...
async function getHtmlFromUrlAsync(url) {
  // is een asynchrone function (lees hieronder waarom)
  //console.log("url = ", url)
